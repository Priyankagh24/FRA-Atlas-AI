...
def find_eligible_people_by_scheme(scheme, village=None, district=None, state=None):
    """
    Returns people who have already been ASSIGNED this scheme (via eligible_scheme column),
    not people who theoretically qualify based on conditions.
    The eligible_scheme is set at upload time by determine_scheme() in eligibility_utils.py,
    which enforces priority rules so each person gets exactly one scheme.
    """

    # Handle queries without scheme (location-only or all claims)
    if scheme is None:
    ...
    # Original scheme-based logic
    scheme_name = scheme.get("name", "").strip()

    # Direct DB query: filter by eligible_scheme column + optional location filters
    base_query = "SELECT * FROM fra_documents WHERE LOWER(eligible_scheme) = LOWER(:scheme_name)"
